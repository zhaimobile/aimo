...
#warning ;
    double theta = 0.1;//RandFloat() * TwoPi;
...
Vector2D SteeringBehavior::Seek(Vector2D TargetPos)
{
    ...
    //printf("Velocity: V(%f,%f)   DesiredVelocity: V(%f,%f)\n",TargetPos.x- m_pVehicle->Pos().x, TargetPos.y-m_pVehicle->Pos().y, Vec2DNormalize(TargetPos - m_pVehicle->Pos()).x, Vec2DNormalize(TargetPos - m_pVehicle->Pos()).y);
                                  ...
                                  Deceleration deceleration)
{
    ...
    if (dist > 0)
    {
        ...
        if (speed > m_pVehicle->MaxSpeed()) {
            speed = m_pVehicle->MaxSpeed();
        }
